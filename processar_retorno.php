...
try {
    ...
    $medico_id_retorno = (int)($agenda['MEDICO_ID'] ?? 0);
    $esp_agenda = !empty($agenda['ESPECIALIDADE_ID']) ? (int)$agenda['ESPECIALIDADE_ID'] : 0;

    if ($medico_id_retorno > 0 && $especialidade_id > 0) {
        ...
        $result_validar_esp = ibase_execute($stmt_validar_esp, $medico_id_retorno, $especialidade_id);
        $row_validar = ibase_fetch_assoc($result_validar_esp);

        if ($row_validar['EXISTE'] > 0) {
            error_log("[RETORNO] ESPECIALIDADE_ID $especialidade_id é válida para o médico.");
        } else {
            error_log("[RETORNO] ESPECIALIDADE_ID $especialidade_id NÃO pertence ao médico. Usando da agenda ($esp_agenda).");
            $especialidade_id = $esp_agenda;
        }
    } elseif ($especialidade_id <= 0 && $esp_agenda > 0) {
        error_log("[RETORNO] Nenhuma especialidade enviada. Usando da agenda ($esp_agenda).");
        $especialidade_id = $esp_agenda;
    }
