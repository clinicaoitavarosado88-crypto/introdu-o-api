...
// Buscar tipo da agenda para decidir se busca exames
$query_tipo_agenda = "SELECT TIPO FROM AGENDAS WHERE ID = ?";
$stmt_tipo = ibase_prepare($conn, $query_tipo_agenda);
$result_tipo = ibase_execute($stmt_tipo, $agenda_id);
$row_tipo = ibase_fetch_assoc($result_tipo);
$tipo_agenda = trim($row_tipo['TIPO'] ?? '');

// Buscar exames APENAS para agendas de procedimento (consultas não têm exames)
foreach ($agendamentos as $hora => $agendamento) {
    if ($agendamento['numero'] && $tipo_agenda === 'procedimento') {
