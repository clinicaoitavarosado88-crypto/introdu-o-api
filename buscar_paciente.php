...
try {
    ...
    // Formato curto sem separadores: ddmmyy (só se for data válida)
    else if (preg_match('/^(\d{2})(\d{2})(\d{2})$/', $termo, $matches_data)) {
        $dia = (int)$matches_data[1];
        $mes = (int)$matches_data[2];
        if ($mes >= 1 && $mes <= 12 && $dia >= 1 && $dia <= 31) {
            $eh_busca_data = true;
            $ano = '19' . $matches_data[3];
            $data_busca = $ano . '-' . sprintf('%02d', $mes) . '-' . sprintf('%02d', $dia);
        }
    }
    ...
    if ($eh_busca_multipla) {
        ...
        $sql .= "(
                ...
                OR p.CPF STARTING WITH ?                          /* CPF com formatação */
                OR p.CPF STARTING WITH ?                          /* CPF sem formatação */
                OR REPLACE(REPLACE(REPLACE(p.CPF, '.', ''), '-', ''), '/', '') STARTING WITH ?  /* BD formatado, busca sem */
                OR REPLACE(REPLACE(REPLACE(p.CPF, '.', ''), '-', ''), '/', '') STARTING WITH ?  /* BD formatado, busca limpa */
            )";
    }
    ...
    if ($eh_busca_multipla) {
        ...
        $params = [$termo_upper, $termo_upper, $termo_upper, $termo_upper, $termo, $termo_limpo_cpf, $termo, $termo_limpo_cpf];
        ...
        $params = array_merge($params, [$termo_upper, $termo_upper, $termo_upper, $termo, $termo_limpo_cpf, $termo, $termo_limpo_cpf, $termo_upper]);
