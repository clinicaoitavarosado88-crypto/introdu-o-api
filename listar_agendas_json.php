...
try {
    ...
    if ($tipo === 'consulta') {
        ...
        $sql = "
            ...
            JOIN LAB_MEDICOS_ESPECIALIDADES me ON me.MEDICO_ID = m.ID AND me.ESPECIALIDADE_ID = $id
            JOIN ESPECIALIDADES e ON e.ID = me.ESPECIALIDADE_ID
            JOIN LAB_CIDADES u ON u.ID = a.UNIDADE_ID
            WHERE a.TIPO = 'consulta' AND a.SITUACAO = 1 $whereDia $whereCidade
        ";
        ...
        $sql = "
            ...
            WHERE a.TIPO = 'procedimento' AND a.SITUACAO = 1 AND p.ID = $id $whereDia $whereCidade
        ";
    ...
    while ($a = ibase_fetch_assoc($res_agendas)) {
        ...
        $agenda_data['configuracoes'] = [
            ...
            'tipo_agenda' => !empty($a['TIPO_AGENDA']) ? mb_convert_encoding(trim($a['TIPO_AGENDA']), 'UTF-8', 'Windows-1252') : null
        ];
