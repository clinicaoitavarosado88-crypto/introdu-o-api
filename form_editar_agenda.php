...
<form method="post" id="form-agenda" class="space-y-8">
    ...
    <div id="campo-especialidade" style="display: none;">
        <label class="block text-sm font-medium text-gray-700">Especialidade <span class="text-red-500">*</span></label>
        <select name="especialidade_id" id="select-especialidade" data-erro="Informe a especialidade"
        class="select2-tailwind w-full border border-gray-300 rounded px-3 py-2 text-sm">
        <option value="">Selecione uma especialidade</option>
        <?php
        $e = ibase_query($conn, "SELECT ID, NOME FROM ESPECIALIDADES ORDER BY NOME");
        while ($r = ibase_fetch_assoc($e)) {
            $selected = (isset($agenda['ESPECIALIDADE_ID']) && $agenda['ESPECIALIDADE_ID'] == $r['ID']) ? 'selected' : '';
            $nome = htmlspecialchars(mb_convert_encoding($r['NOME'], 'UTF-8', 'Windows-1252'));
            echo "<option value='{$r['ID']}' $selected>$nome</option>";
        }
        ?>
